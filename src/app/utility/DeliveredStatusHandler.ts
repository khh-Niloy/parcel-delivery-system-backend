...
export const DeliveredStatusHandler = async(parcel: IParcel, payload: {status: Status})=>{
    ...
    await Parcel.findOneAndUpdate({trackingId: parcel.trackingId}, {
    ...
    // console.log("update delivery agent available status")
    ...
    try {
        ...
        if (allAvailableDeliveryAgent.length > 0) {
            ...
            while (allPendingParcels.length > 0 && allAvailableDeliveryAgent.length > 0) {
                ...
                const {selectedDeliveryAgent} = await getETA(selectedPendingParcel?.pickupAddress.latitude as number, selectedPendingParcel?.pickupAddress.longitude as number, 
